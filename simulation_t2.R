...
generate_Y <- function(n, p, Gamma) {
  X <- generate_X_on_sphere(n, p)
  Y <- matrix(nrow = n, ncol = p)
  for (j in 1:n) {
    # Generate rho_j from the chi-squared distribution
    rho_j_squared <- rchisq(1, df = p)
    ...
    # Assign y_j without scaling to unit norm
    Y[j, ] <- y_j
  }
  return(Y)
}
...
system.time({
  ...
  generate_TL <- function(n, p, theta1, eta, scenario) {
    Y <- generate_samples(n, p, theta1, eta, scenario)
    R_n <- cor(Y) # 计算样本相关性矩阵
  ...
  test_statistics_T_L <- replicate(num_tests, {
    generate_TL(n, p, theta1, eta, scenario)
  })
  # 计算均值
